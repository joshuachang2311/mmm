...
if __name__ == '__main__':
    print("Unison MIDI sequence generator")
    base_duration = int(480 * 4 / int(input("Note value? 1 for whole note, 2 for half note, etc. ")))
    base_pitch = int(input("Note pitch? 60 is C4, 61 is C#4, etc. "))
    ...
    for _ in range(n_notes):
        midi_obj.instruments[0].notes.append(
            containers.Note(velocity=64, pitch=base_pitch, start=current_duration, end=current_duration + base_duration))
